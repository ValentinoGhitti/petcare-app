<template>
  <v-container fluid>
    <v-row class="d-flex justify-start align-center min-height">
      <v-col class="test"></v-col>
...
export default {
  ...
  methods: {
    async validateEmail() {
      ...
      if (isValidStep1) {
        try {
          await this.$store.dispatch('resetPasswordAction', { email: this.email });
          this.step = 2;
        } catch (error) {
          console.log('The email you entered does not exist');
          this.step = 1;
        }
      } else {
    ...
    async resetPassword() {
      ...
      if (isValidStep2) {
        if (this.newPassword !== this.repeatPassword) {
          ...
          try {
            await this.$store.dispatch('resetPasswordAction', { email: this.email, newPassword: this.newPassword });
...
.login-card {
  ...
  border-radius: 20px;
  box-shadow: 0 10px 20px rgba(0, 0, 0, 0.1);
  background: white; /* Fondo blanco para la tarjeta de login */
}
